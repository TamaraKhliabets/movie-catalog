...
function DataMovie({ movie }) {
  ...
  const year = (release_date || first_air_date)
    ? new Date(release_date).getFullYear() || new Date(first_air_date).getFullYear() : null;
  ...
  if (episode_run_time) {
    episode_run_time.sort((a, b) => a - b).splice(1, episode_run_time.length - 2);
  }
