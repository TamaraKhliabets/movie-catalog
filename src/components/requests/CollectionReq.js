...
export default class CollectionReq extends Component {
  ...
  render() {
    ...
    if (!collection) return <div className="null" />;

    const {
      name, overview, poster_path, parts,
    } = collection;
    const src = 'https://image.tmdb.org/t/p/w342'.concat(poster_path);

    const nodeItem = parts.map(e => (
      <div key={e.id} className="list_item"><MovieLink movie={e} /></div>
    ));
...
CollectionReq.propTypes = {
  url: PropTypes.string,
  collection: PropTypes.instanceOf(Array),
  collectionFetchData: PropTypes.func.isRequired,
};

CollectionReq.defaultProps = {
  url: '',
  collection: null,
};
